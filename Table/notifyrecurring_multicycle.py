...
class NotifyRecurringMC:
    def __init__(self,notifyrecurringMCObject):
        self.dictNotifyRecurringMC = notifyrecurringMCObject
        self.logger = logging.getLogger(self.__class__.__name__)

    def insert(self, db):
        self.logger.debug("Insert into table [" + self.dictNotifyRecurringMC["table"] + "] with offer_external_id: [" + str(self.dictNotifyRecurringMC["offer_external_ID"]) + "]")
...
if __name__ == '__main__':
    ...
    with open("Table/templatenotifyrecur_multicycle.yml", "r") as stream:
        try:
            ...
            for i in range(0,len(dictConfig)):
                ...
                if dictConfig[i]["AutoInteractDB"]["table"] == "NotifyRecurring_MultiCycle":
                    ...
                    notifyrecurringMC = NotifyRecurringMC(dictConfig[i]["AutoInteractDB"])
                    notifyrecurringMC.run(db)
            print("Finish")
        except yaml.YAMLError as exc:
            print(exc)
    exit(0)